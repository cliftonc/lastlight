...
| `dependabot-ci-fix.md` | Fix-only: first merge the base branch into a red dependency-update PR (plain, no force-push — so a `behind` PR is made current), then make the smallest fix (lockfile / call-site / type), run the gate, and push. It does **not** classify, label, or merge — once the push turns checks green the `pr.checks_passed` webhook hands off to `dependabot-pr-merge` (the single owner of the classify → label → auto-merge decision). If it can't fix with a small safe change it stops and applies the `requires-human` label so the nightly red-PR sweep won't re-attempt it. **Triggered by the `pr.checks_failed` webhook, or by the daily `fix-red-dependency-prs` cron whose runner finds settled-red Dependabot / Renovate PRs in code (`src/cron/dependabot-discovery.ts`) and fans out one bounded run per PR.** | Commits on PR branch / labels (give-up only) |
