...
| `dependabot-ci-fix.md` | Read the CI failures on a red dependency-update PR, make the smallest fix (lockfile / call-site / type), run the gate, push. If it can't fix with a small safe change it stops and applies the `requires-human` label so the nightly red-PR sweep won't re-attempt it. **Triggered by the `pr.checks_failed` webhook, or by the daily `fix-red-dependency-prs` cron whose runner finds settled-red Dependabot / Renovate PRs in code (`src/cron/dependabot-discovery.ts`) and fans out one bounded run per PR.** | Commits on PR branch / labels |
| `dependabot-assess.md` | Assess the pushed diff; if trivial, call `github_enable_auto_merge` (merge-once-green), else comment for a human. Records the verdict as a label — `dependency-trivial` (clearing any stale `requires-human`) or `dependency-functional` + `requires-human`. | Enables auto-merge / posts a comment / labels |
