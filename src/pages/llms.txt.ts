...
const EXTERNAL: Array<{ url: string; label: string; description?: string }> = [
  { url: 'https://github.com/cliftonc/lastlight', label: 'GitHub repository', description: 'Source code, issues, and releases' },
...
export const GET: APIRoute = async ({ site }) => {
  ...
  const lines: string[] = [
    ...
    '',
    'Last Light is a YAML-driven workflow engine that runs Claude in Docker sandboxes with downscoped GitHub tokens. It triages issues, reviews PRs, fixes small things, runs scheduled repo-health reports, and chats over Slack — handing work between Architect / Executor / Reviewer roles and pausing at approval gates the maintainer can resume from a comment. The harness is the product: the workflows are YAML you can fork and edit; the agent runtime, sandboxing, and policy gates are the engine.',
    '',
    '## Site',
    '',
    ...SITE_PAGES.map((p) => {
