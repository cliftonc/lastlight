...
import { api, type ConfigBundle, type OverridesBundle, type OverlayAsset } from "../api";

type Pane = "default" | "overlay" | "merged" | "sources" | "overrides";
...
export function ConfigPage() {
  ...
  const value = config && pane !== "overrides" ? config[pane] : null;
    ...
    <div className="flex flex-col flex-1 overflow-hidden bg-base-100">
      <div className="border-b border-base-300 px-4 py-3">
        ...
        <p className="text-xs text-base-content/60 mt-1">
          ...
          Overrides pane lists assets the deployment overlay forks.
        </p>
      </div>
      <div className="flex gap-1 border-b border-base-300 bg-base-200/60 px-4 py-2">
        {(["default", "overlay", "merged", "sources", "overrides"] as const).map((id) => (
          <button
            key={id}
            onClick={() => setPane(id)}
